...
module.exports = {
    validateRegister: (req, res, next) => {
        if(!req.body.username || req.body.username < 5) {
            return res.status(400).send({msg: "Please enter a username with min 5 chars"});
        }

        if(!req.body.password || req.body.password < 6) {
            return res.status(400).send({msg: "Please enter a password with min 6 chars"});
        }
    ...
    alreadyExists: (req, res, next) => {
        db.query(`SELECT * FROM users WHERE LOWER(username) = LOWER(${db.escape(req.body.username)});`, (err, result) => {
            if(result.length){
                return res.status(409).send({msg: `Username ${req.body.username} already exists`});
            } else {
                next();
            }
        });
